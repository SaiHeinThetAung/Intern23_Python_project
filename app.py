...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:hhhh@localhost:2345/finalticket'
...
def home():
    ...
    if session and session['username']:
        ...
        #    user_order = Users.query.filter_by(username = validate_user).first()
        #    username_order = Orders.query.filter_by(user_name = validate_user,user_id = user_order.id).first()
        if orders:
            for order in orders:
        ...
        if session and session['username']:
            ...
            if validUser:
                ...
                return render_template('order.html', username=validUser, tickets=form_selected_tickets,
                                       totalprice=totalprice, price=price, orderId=order_id)
            else:
...
def check(checkid):
    if session and session['username']:
        ...
        if not data.verify:
            return render_template('check.html', username=validate_user, checking=None, data=data)
        ...
        return render_template('check.html', checking=data.verify, username=validate_user)
